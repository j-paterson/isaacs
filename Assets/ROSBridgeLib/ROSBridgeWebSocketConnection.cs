 ...
 */
    ...
    /* NOTE: Grey code runs on the hololens only. 
     * 
     */ 
...
#endif
#if !UNITY_EDITOR
        //PUT WINDOWS NETWORKING CODE HERE
...
#if !UNITY_EDITOR
            foreach(Type p in _subscribers) {
                WebSock_SendMessage((ROSBridgeMsg.UnSubscribe(GetMessageTopic(p))));
				//Debug.Log ("Sending " + ROSBridgePacket.unSubscribe(getMessageTopic(p)));
			}
			foreach(Type p in _publishers) {
                WebSock_SendMessage((ROSBridgeMsg.UnAdvertise(GetMessageTopic(p))));
				//Debug.Log ("Sending " + ROSBridgePacket.unAdvertise (getMessageTopic(p)));
			}
            messageWebSocket.Dispose();
            messageWebSocket.Close(1000, "Closing");
...
/* Used to establish connection to host ip address and port number
 * 
 */ 
        private void Run()
        {
            ...
            messageWebSocket = new MessageWebSocket();
                messageWriter = new DataWriter(messageWebSocket.OutputStream);
                 ...
                 server = new Uri("ws://" + _host + ":" + _port.ToString());
                     
                 //Add the MessageReceived event handler.
                 messageWebSocket.MessageReceived += OnMessage;
...
/* This function is called when the Websocket finishes its connection attempt
 * 
 */  
#if !UNITY_EDITOR
        
        public void NetworkConnectedHandler(IAsyncAction asyncInfo, AsyncStatus statusNet)
     {
        // Status completed is successful.
        if (statusNet == AsyncStatus.Completed)
        {
             status += "1";
            foreach (Type p in _subscribers)
                {
                WebSock_SendMessage(ROSBridgeMsg.Subscribe(GetMessageTopic(p), GetMessageType(p)));

            }
            foreach (Type p in _publishers)
               {
                        
                    WebSock_SendMessage(ROSBridgeMsg.Advertise(GetMessageTopic(p), GetMessageType(p)));
            }
        }
        else
        {
            Exception exception  = asyncInfo.ErrorCode; 
            status += "f" + exception;
        }
     }

#endif
/* This function is called when the websocket object recieves a message from a client. 
 * 
 */ 
        ...
        private void OnMessage(MessageWebSocket sender, MessageWebSocketMessageReceivedEventArgs args)
        {
            ...
            try
            {
                using (DataReader reader = args.GetDataReader())
                {
                    ...
                    s = reader.ReadString(reader.UnconsumedBufferLength);
                }
            ...
            {
                WebErrorStatus webstatus = WebSocketError.GetStatus(ex.GetBaseException().HResult);
...
/* Called when unity recieves a message s from a client. 
 * 
 */ 
        ...
        /* publish a msg under given topic name
         */ 
   ...
   //Add code here to do something when the connection is closed locally or by the server
        ...
        /* Does the actual sending. If the message was sent successfully the status would append an S,
         * otherwise it will append an n.
         */ 

#if !UNITY_EDITOR
    private void WebSock_SendMessage(string message)
    {
